...
interface ProductToPublish {
  ...
  shopify_taxonomy_category?: string;  // ✅ NUEVO: Taxonomía estándar de Shopify
  etiquetas?: string[];
  colecciones?: string[];
}
...
interface ShopifyProduct {
  ...
  vendor: string;
  product_type: string;
  product_category?: string;  // ✅ NUEVO: Taxonomía estándar de Shopify
  tags: string;
  variants: {
    price: string;
    sku: string;
...
async function getOrCreateCollection(
  ...
  try {
    ...
    const searchResponse = await fetch(
      `https://${shopifyDomain}/admin/api/2024-10/custom_collections.json?title=${encodeURIComponent(collectionTitle)}`,
      {
        headers: {
    ...
    const createResponse = await fetch(
      `https://${shopifyDomain}/admin/api/2024-10/custom_collections.json`,
      {
        method: 'POST',
        headers: {
          'X-Shopify-Access-Token': shopifyAccessToken,
...
async function addProductToCollection(
  ...
  try {
    const response = await fetch(
      `https://${shopifyDomain}/admin/api/2024-10/collects.json`,
      {
        method: 'POST',
        headers: {
          'X-Shopify-Access-Token': shopifyAccessToken,
  ...
  try {
    ...
    for (const product of products as ProductToPublish[]) {
      try {
        ...
        // ✅ Agregar taxonomía de Shopify si existe
        if (product.shopify_taxonomy_category && product.shopify_taxonomy_category.trim() !== '') {
          shopifyProduct.product_category = product.shopify_taxonomy_category;
          console.log(`📊 Asignando taxonomía: "${product.shopify_taxonomy_category}"`);
        }
        ...
        console.log('📤 Publicando producto en Shopify:', {
          ...
          product_category: shopifyProduct.product_category || 'No asignada',
        });

        // Llamada a Shopify Admin API para crear producto
        const response = await fetch(
          `https://${shopifyDomain}/admin/api/2024-10/products.json`,
          {
            method: 'POST',
            headers: {
              'X-Shopify-Access-Token': shopifyAccessToken,
        ...
        // ✅ Si se envió taxonomía, actualizarla nuevamente para confirmarla
        // Esto evita que Shopify la muestre solo como "sugerencia"
        if (product.shopify_taxonomy_category && product.shopify_taxonomy_category.trim() !== '') {
          try {
            console.log(`🔄 Confirmando taxonomía para producto ${createdProductId}...`);
            
            const updateResponse = await fetch(
              `https://${shopifyDomain}/admin/api/2024-10/products/${createdProductId}.json`,
              {
                method: 'PUT',
                headers: {
                  'X-Shopify-Access-Token': shopifyAccessToken,
                ...
                body: JSON.stringify({
                  product: {
                    id: createdProductId,
                    product_category: product.shopify_taxonomy_category,
                  },
                }),
              }
            );
            ...
            if (updateResponse.ok) {
              console.log(`✅ Taxonomía confirmada para producto ${createdProductId}`);
            } else {
              const updateData = await updateResponse.json();
              console.warn(`⚠️ No se pudo confirmar taxonomía:`, updateData);
            }
          } catch (updateError) {
            console.warn(`⚠️ Error al confirmar taxonomía:`, updateError);
