  ...
  const generateSEO = async () => {
    ...
    try {
      ...
      for (const productId of selectedProducts) {
        ...
        if (!response.ok) throw new Error('Error al generar SEO');
        const data = await response.json();
        newSeoData[productId] = data;
      }

      setSeoData(newSeoData);
      showMessage('success', `SEO generado para ${selectedProducts.length} producto(s)`);
    } catch (error: any) {
      console.error('Error generating SEO:', error);
      showMessage('error', 'Error al generar SEO con IA');
    } finally {
      ...
      for (const productId of selectedProducts) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            errors.push(`${product.product_name}: Ya existe en Shopify`);
            errorCount++;
          } else {
            errors.push(`${product.product_name}: ${data.error}`);
            errorCount++;
          }
        } catch (error: any) {
