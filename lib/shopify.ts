...
export async function createShopifyCollection(title: string) {
  try {
    const response = await shopifyClient.post('/custom_collections.json', {
      custom_collection: {
        title: title,
        published: true,
      },
    ...
    console.error('Error creating Shopify collection:', error.response?.data || error)
    throw error
  }
}
