...
import { Product, ProductTableRow, COLECCIONES_SHOPIFY, SHOPIFY_TAXONOMY_CATEGORIES } from '@/types/product';
...
export default function ProductsTable({ initialProducts }: ProductsTableProps) {
  ...
  useEffect(() => {
    setProducts(
      initialProducts.map(p => ({
        ...
        shopify_taxonomy_category: p.shopify_taxonomy_category || '',
  ...
  const handleSaveProduct = async (product: ProductTableRow) => {
    ...
    try {
      ...
      // ✅ NUEVO: Guardar taxonomía de Shopify
      if (product.shopify_taxonomy_category !== undefined && product.shopify_taxonomy_category !== null) {
        updateData.shopify_taxonomy_category = product.shopify_taxonomy_category;
      }
  ...
  const handlePublishSelected = async () => {
    ...
    if (productsWithoutType.length > 0) {
      alert(`❌ Hay ${productsWithoutType.length} producto(s) sin tipo de producto.\n\nPor favor completa este campo antes de publicar.`);
      return;
    }
    ...
    setPublishing(true);
    try {
      console.log('📤 Publicando productos en Shopify:', selectedProducts.map(p => ({
        sku: p.sku,
      ...
      // Mostrar resultados
      const successMessage = `✅ Publicación completada:\n\n` +
        ...
        (data.errors.length > 0 
          ? `\n\nErrores:\n${data.errors.map((e: any) => `✗ ${e.product_name}: ${JSON.stringify(e.error)}`).join('\n')}`
          : '');
                  ...
                  <tr key={product.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 editable-cell">
                      <div className="relative">
                        <input
                          type="text"
                          list={`taxonomy-${product.id}`}
                          value={product.shopify_taxonomy_category || ''}
                          onChange={(e) => handleUpdateField(product.id, 'shopify_taxonomy_category', e.target.value)}
                          placeholder="Ej: Electronics > Computers"
                          className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        />
                        <datalist id={`taxonomy-${product.id}`}>
                          {SHOPIFY_TAXONOMY_CATEGORIES.map((cat) => (
